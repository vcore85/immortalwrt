...
if [ $CSQ -ge 0 -a $CSQ -le 31 ]; then
	...
	O_CSQ=$(sms_tool -d $DEVICE at "AT+GTCCINFO?")
	CSQ=$(echo "$O_CSQ" | awk '/NR service cell:/{getline; print}' | awk -F[,] '{print $12}')
	if [ -z $CSQ ]; then
		CSQ=$(echo "$O_CSQ" | awk '/GTCCINFO:/{getline; print}' | awk -F[,] '{print $12}')
	fi
	CSQ_PER=$(((CSQ*100)/126))
fi
...
COPS_NUM=$(echo "$O" | awk -F[\"] '/^\+COPS:.+,2/ {if (!matched) {print $2; matched=1}}')
if [ -n "$COPS_NUM" ]; then
...
# CGREG
eval $(sms_tool -d $DEVICE at "at+cgreg?" | awk -F[,] '/^\+CGREG/ {gsub(/[[:space:]"]+/,"");printf "T=\"%d\";LAC_HEX=\"%X\";CID_HEX=\"%X\";LAC_DEC=\"%d\";CID_DEC=\"%d\";MODE_NUM=\"%d\"", $2, "0x"$3, "0x"$4, "0x"$3, "0x"$4, $5}')
case "$T" in
	0*)
		REG="0";;
	1*)
...
if [ -z "$REG" ]; then
REG=$(sms_tool -d $DEVICE at "at+cgreg?" | awk -F[,] '/^\+CGREG/ {print $2;exit}')
fi
